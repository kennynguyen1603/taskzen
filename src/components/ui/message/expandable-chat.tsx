...
const chatConfig = {
  ...
  states: {
    ...
    closed: 'pointer-events-none opacity-0 invisible scale-95 sm:translate-y-3'
  }
}
...
const ExpandableChat: React.FC<ExpandableChatProps> = ({
  ...
  const toggleChat = () => {
    ...
    }, 350) // Match this with the duration-350 class below
  }
    ...
    <div className={cn(`fixed ${chatConfig.positions[position]} z-50`, className)} {...props}>
      <div
        ...
        className={cn(
          ...
          'transition-all duration-350 ease-spring transform',
          isOpen ? chatConfig.states.open : chatConfig.states.closed,
          isAnimating && 'will-change-transform will-change-opacity',
          className
        )}
        style={{
...
const ExpandableChatToggle: React.FC<ExpandableChatToggleProps> = ({
  ...
  <Button
    ...
    onClick={toggleChat}
    className={cn(
      'w-14 h-14 rounded-full shadow-md flex items-center justify-center hover:shadow-lg hover:shadow-black/30 transition-all duration-300 transform hover:scale-105',
      isOpen && 'bg-primary-foreground text-primary border-primary', // Change styling when open
      className
    )}
    {...props}
  >
